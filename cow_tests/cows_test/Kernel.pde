...
////////////////////////////////////////////////////////////////
    PVector getMin(float[][] matrix, PVector loc)
    {
        ...
        for( int j = 0; j < nHood[i].length; j++){
            ...
            // if they are equal choose the closer one
            if( nHood[i][j].z == minVal && nHood[i][j].z != NaN )
            {
                minVal  = nHood[i][j].z;
                
                float dist1 = dist( loc.x, loc.y, minValx, minValy );
                float dist2 = dist( loc.x, loc.y, nHood[i][j].x, nHood[i][j].y );
    ...
    PVector getMax(float[][] matrix, PVector loc )
    {
        ...
        for( int j = 0; j < nHood[i].length; j++){
            ...
            if( nHood[i][j].z == maxVal && nHood[i][j].z != NaN )
            {
                ...
                float dist1 = dist( loc.x, loc.y, maxValx, maxValy );
                float dist2 = dist( loc.x, loc.y, nHood[i][j].x, nHood[i][j].y );
